...
export class TextsService {
  ...
  getTextByLanguage(language: string): Observable<Text> {
    return this._httpService.get<Text>(`api/texts?language=${language}`);
  }

  getTextById(id: number): Observable<Text> {
    return this._httpService.get<Text>(`api/texts/${id}`);
  }

  getPagedTexts(paginatedRequest: PaginatedRequest): Observable<PagedResult<ITextGridRow>> {
    console.log(paginatedRequest);
    return this._httpService.post<PagedResult<ITextGridRow>>('api/texts/paginated', paginatedRequest);
  }

  // POST
  createText(command: CreateTextCommand): Observable<any> {
    return this._httpService.post<any>("/api/texts", command);
  }

  // DELETE
  deleteTextById(id: number): Observable<any> {
    return this._httpService.delete<any>(`/api/texts/${id}`);
  }

  // PUT
  updateText(command: UpdateTextCommand): Observable<any> {
    return this._httpService.put<any>("api/texts", command);
  }
}
